...
2026-07-19 현재 이 문서를 기준으로 초기 구현과 결함 보강을 코드에 반영했다. 이 문서는
...
사용자가 명시적으로 `/usememory`를 켠 채팅에서만, 실제 사용자 메시지와 실제
최종 Assistant 응답을 도구 수행 내역 없이 안전한 plain text로 누적하고, AI
Agent가 필요할 때만 이 기록을 자율적으로 검색해 참고하도록 한다.
...
- 다시 ON으로 바꾸면 기존에 저장된 같은 채팅의 기록을 다시 참고할 수 있다.
...
- import하지 않는다.
- fallback으로 검색하지 않는다.
- 기존 파일 존재 여부로 `/usememory`를 자동 활성화하지 않는다.
...
1. 채팅별 `/usememory` 토글을 제공한다.
2. 누락된 설정의 기본값을 OFF로 해석한다.
...
명확히 표시되어야 한다. ON 응답은 plain-text 장기 저장이라는 opt-in 의미를,
OFF 응답은 기존 record가 삭제되지 않는다는 점을 함께 알린다.
...
- 같은 bot/chat 범위에 이미 저장된 기록을 Agent가 참고할 수 있다.
...
필드가 없거나 해당 chat id가 없으면 OFF다.

실제 bot identity가 다른 두 bot은 같은 chat id를 사용하더라도 설정과 저장 record를
공유하지 않는다. Telegram의 같은 bot이 secret suffix만 회전한 경우 memory record
scope는 numeric bot id로 다시 연결된다. 단, 기존 `bot_settings.json`의 설정 key는
별도 기존 정책을 따르므로 token 회전 뒤 `/usememory`를 다시 켜야 할 수 있다.
...
- 그룹에서 기능이 켜지면 그 메모리는 개인 메모리가 아니라 해당 bot과 group
  chat 범위의 공유 대화 메모리로 해석한다.
...
```text
/usememory — Toggle persistent conversation memory (default: OFF)
```
...
- `v1`: 저장 layout/schema version
- `bot-key-hash`: Telegram token의 secret suffix가 아니라 rotation 후에도 유지되는
  numeric bot id를 memory 전용 domain으로 full-width SHA-256한 opaque scope다.
  raw token과 raw bot id는 disk path에 남지 않는다. 표준 `id:secret` 형식이 아닌
  token(bridge token 포함)은 전체 값 hash로 fail-safe fallback하므로, 그런 token의
  값 자체가 회전하면 자동 재연결은 보장하지 않는다.
- `chat-id`: 1:1 또는 group chat 범위 분리
- `turns`: 실제 완료 대화 턴만 포함
...
### 9.3 chat 범위를 primary memory scope로 선택하는 이유

영구 메모리는 provider session보다 오래 살아야 한다. 따라서 session id 또는
현재 workspace만으로 저장소를 분리하지 않는다.

같은 chat에서 `/start`로 session이나 working directory를 바꾸더라도 사용자의
선호, 이전 결정, 장기 대화 맥락을 다시 찾을 수 있어야 한다. 그래서 bot+chat을
hard isolation boundary로 사용하고, session/workspace는 record metadata로만
남긴다.

이 선택의 의미는 다음과 같다.

- 같은 bot/chat의 여러 provider session은 memory를 공유한다.
- 같은 bot/chat의 여러 workspace는 memory를 공유한다.
- 서로 다른 chat은 memory를 공유하지 않는다.
- 서로 다른 bot은 memory를 공유하지 않는다.
- group chat memory는 group 단위다.
...
system prompt에는 현재 bot/chat의 `turns` root만 넣는다.

```text
~/.cokacdir/memory_store/v1/bots/<this-bot>/chats/<this-chat>/turns/
```

다른 bot 또는 chat의 parent directory를 검색하라고 지시하지 않는다. Agent는
제공된 root 밖의 memory record를 현재 사용자에게 인용하면 안 된다.
...
이것은 중요한 논리적 isolation이지만, 같은 OS 사용자 권한으로 전체 파일 시스템
도구를 실행하는 Agent에 대한 강한 보안 sandbox는 아니다. 이에 대한 제한은 보안
절에서 별도로 다룬다.
...
- `working_directory`: 여러 session/workspace를 공유하는 chat memory에서 작업 맥락
...
group chat이면 JSON payload decoding 규칙, optional `user_label`의 비고유성, 서로
다른 참여자의 기록을 현재 화자에게 섣불리 귀속하지 말라는 규칙도 포함한다.
...
Read-only conversation records are stored under:
<CURRENT_CHAT_MEMORY_ROOT>
...
the store. Never inspect or disclose memory outside the exact root above.
...
```bash
rg -l -i '배포|프로덕션|릴리스|반영' <CURRENT_CHAT_MEMORY_ROOT>
```
...
- ping은 그 공통 protocol을 따라 현재 chat memory를 읽을 수 있다.
...
- ON이면 현재 bot/chat memory root를 계산한다.
...
- field가 없거나 malformed이면 빈 map으로 해석해 OFF로 fail-closed한다.
...
2. 즉시 lock 해제
3. ON 전환이면 추적되는 blocking task에서 full atomic capability probe 수행
...
기능은 기본 OFF다. 이는 자동 장기 저장이 개인정보 관점에서 명시적 선택이어야
하기 때문이다.

ON 상태에서는 User/Assistant text가 디스크에 장기 보존된다는 점을 사용자가
명확히 이해할 수 있어야 한다.
...
따라서 `/usememory` ON은 해당 chat의 향후 User/Assistant 최종 text가 저장된다는
명시적인 opt-in이다. 민감 정보 삭제와 selective retention은 별도 기능으로
설계해야 한다.
...
### 17.4 scope isolation

bot hash와 chat id를 storage path에서 분리한다. 현재 prompt에는 현재 chat root만
노출한다.

그러나 Codex, Agy, OpenCode 등 일부 Agent가 같은 OS 사용자 권한으로 native/full
filesystem access를 가진다면 prompt만으로는 강한 security boundary를 만들 수
없다. Agent가 의도적으로 parent directory를 탐색하면 다른 file을 볼 수 있는
환경일 수 있다.

초기 설계의 보장은 다음 수준이다.

- 애플리케이션이 다른 chat record를 자동 주입하지 않는다.
- prompt가 정확한 현재 scope만 지시한다.
- Agent에게 scope 밖 memory를 읽거나 공개하지 말라고 강하게 지시한다.
- 저장 path가 accidental collision을 방지한다.

강한 강제 격리가 필요해지면 다음 후속 구조가 필요하다.

- scope-enforcing read-only CLI
- capability token
- 별도 process/user
- sandbox mount
- provider별 tool policy enforcement
...
- no-overwrite writer
- private directory
- 향후 필요 시 scope-enforcing wrapper
...
새 workspace 또는 provider session을 시작해도 같은 bot/chat memory를 계속
사용한다. record의 working directory를 참고해 현재 프로젝트와 관련된 과거
기록인지 판단한다.

### 18.3 `/model`과 provider switch

provider가 바뀌어도 memory source와 storage format은 바뀌지 않는다. 동일한
bot/chat에서 Claude가 저장한 logical turn을 이후 Codex가 참고할 수 있고 그
반대도 가능하다.
...
5. scope-enforcing search/get CLI
...
- memory root 계산
- raw bot token을 disk에 남기지 않는 full-width memory 전용 bot key 파생
- bot/chat scope path 계산
...
- exact root를 JSON quote한 system prompt guidance 조립
...
- 새 `BotSettings::default()`에서 모든 chat은 memory OFF
- settings JSON에 `use_memory`가 없으면 OFF
- 빈 object면 OFF
- 해당 chat에 true가 있으면 ON
- false가 있으면 OFF
- malformed non-boolean 값은 무시하고 OFF
...
- 같은 bot/chat의 session switch 후 이전 record 발견 가능
- 같은 bot/chat의 provider switch 후 이전 record 발견 가능
- 다른 chat root가 prompt에 나타나지 않음
- 다른 bot root가 prompt에 나타나지 않음
...
1. 설치 또는 기존 설정 file에서 memory 기본값이 OFF다.
...
13. 저장소가 bot/chat별로 분리된다.
14. Agent는 현재 chat root만 안내받는다.
15. 저장이나 검색을 위해 별도 LLM 호출을 하지 않는다.
16. legacy file을 읽거나 migration하지 않는다.
...
22. enable 전 atomic capability probe가 실패하면 ON으로 persist하지 않는다.
...
| prompt에 넣는 것 | 검색 protocol과 현재 chat root만 |
...
| enable 검증 | atomic publish/sync/remove capability probe 성공 필수 |
...
| hard isolation | bot+chat 논리 scope; 강한 sandbox는 후속 과제 |
...
- read-only scope-enforcing `--memory-search`/`--memory-get` CLI
