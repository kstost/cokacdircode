...
| cokacdir persistent memory | 매 실행의 집중 검색과 관련 과거 대화 회상 | 없음 | 구조적으로 저장 불가 | 매 정상 user turn의 User/최종 Assistant만 저장 |
...
아닌 안정적인 `bot_identity`를 함께 저장한다. token은 bot에 접근하는 credential이고,
`bot_identity`가 credential 회전 전후에도 같은 remote bot임을 나타낸다. Telegram은
표준 token의 numeric bot ID, Discord는 인증된 user ID, Slack은 인증된 workspace ID +
bot-user ID를 사용한다.

현재 token key와 정확히 일치하는 valid entry가 있으면 그 entry를 authoritative하게
사용한다. 같은 identity의 이전 token key가 함께 남아 있어도 startup을 막지 않고,
다음 성공적인 settings save에서 현재 key만 남기고 제거한다. 현재 token key가 없을
때는 동일 identity entry가 정확히 하나면 설정을 승계한다. 이 no-exact-entry 경로에서
후보가 여러 개이거나 token/hash/identity가 모순되면 다른 bot의 설정을 선택하지 않고
startup을 fatal로 중단한다. bridge의 과거 entry처럼 안정 ID를 증명할 수 없는 entry는
...
Before answering or taking any action, perform a focused search of this store
for context relevant to the current request. Do this on every enabled run.

Search autonomously with available file listing/search/read tools:
1. Start with a narrow query using several distinctive terms from the current
   request and conversation context.
2. If results are absent or weak, actively retry with synonyms, related nouns,
   alternate wording, broader terms, and another likely date/time range.
3. Read the most relevant candidates without loading the complete store.
4. Prefer direct User statements and clearly applicable prior conclusions.
5. If no relevant record is found, continue normally without mentioning the lookup.
...
### 12.3 모든 ON 실행의 검색 방식

memory가 ON이면 현재 요청의 종류와 관계없이 먼저 focused preflight search를 수행한다.
아래와 같은 과거 맥락이 암시된 요청에서는 관련 record를 찾기 위해 특히 구체적인
검색어와 동의어를 사용한다.
...
검색 자체는 모든 ON 실행에서 수행한다. 다만 관련 record가 없으면 현재 요청만으로
계속 처리하며 검색 사실을 사용자에게 알리지 않는다. 찾은 record도 현재 요청에
실제로 관련된 경우에만 추론과 답변에 반영하고, 현재 system/developer/user 지시와
충돌하면 현재 지시를 우선한다.
...
- exact current-token entry가 있으면 같은 identity의 이전 token entry가 함께 있어도
  current entry를 사용하고 다음 성공적인 저장에서 stale key를 제거
- exact current-token entry가 없을 때 identity 후보가 여러 개이거나
  token/hash/identity가 모순되면 startup fatal
